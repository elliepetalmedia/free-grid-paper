...
export default function Home() {
  ...
  const getPdfFormat = (pageSize: { width: number; height: number }) => {
    ...
    if (standardFormats.includes(lowerPageSize)) {
      return lowerPageSize as 'a4' | 'letter' | 'legal' | 'a0' | 'a1' | 'a2';
    }

    return [pageSize.width, pageSize.height] as [number, number];
  };

  const downloadPDF = async (paperType?: PaperType) => {
    ...
    try {
      const pageSize = getPageDimensions();
      const [{ jsPDF }, { renderPdfPage }] = await Promise.all([
        import('jspdf'),
        import('@/generator/pdf-renderers'),
      ]);
      ...
      renderPdfPage(doc, pageSize.width, pageSize.height, settings, type);
      doc.save(`${type}-${settings.pageSize}-${Date.now()}.pdf`);
    } finally {
  ...
  const downloadBatchPDF = async () => {
    if (settings.batchPaperTypes.length === 0 || isDownloading) return;
    ...
    try {
      const pageSize = getPageDimensions();
      const [{ jsPDF }, { renderBatchPdf }] = await Promise.all([
        import('jspdf'),
        import('@/generator/pdf-renderers'),
      ]);
      ...
      renderBatchPdf(doc, pageSize.width, pageSize.height, settings);
      doc.save(`batch-${settings.pageSize}-${Date.now()}.pdf`);
    } finally {
      setIsDownloading(false);
    }
  };
                ...
                <details className="space-y-2">
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <Button
                      ...
                      disabled={settings.batchPaperTypes.length === 0 || isDownloading}
                      className="w-full mt-2"
                      size="sm"
                      data-testid="button-batch-download"
                    >
                      <Download className="w-4 h-4 mr-2" />
                      {isDownloading ? 'Preparing Batch...' : 'Download Batch'}
                    </Button>
                  </div>
                </details>
              </div>
        ...
        <main className="flex-1 p-2 md:p-8 flex flex-col items-center justify-center bg-background overflow-hidden relative">
          ...
          {quickDownloadText && (
            <div className="w-full max-w-2xl mb-4 bg-primary/10 border border-primary/30 rounded-lg p-4 flex items-center justify-between gap-4" data-testid="quick-download-banner">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={() => downloadPDF()} disabled={isDownloading} size="sm" data-testid="button-quick-download">
                  <Download className="w-4 h-4 mr-2" />
